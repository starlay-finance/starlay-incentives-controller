...
task(
  'update-deposit-borrow-weight',
  'Update deposit borrow weight to PullRewardsIncentivesController'
  ...
  const { incentiveControllerProxy, weight } = getIncentivesConfigPerNetwork(network);
  ...
  const tx = await incentivesControllerProxy.setDepositBorrowWeight(weight.deposit, weight.borrow);
  await waitForTx(tx);
});
